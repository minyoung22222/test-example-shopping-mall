  ...
  mockUseCartStore({
    cart: {
      6: {
        ...
        title: 'IU SHOPPER',
        price: 417,
        description: 'IU SHOPPER IU SHOPPER IU SHOPPER IU SHOPPER IU SHOPPER',
        images: [
      ...
      8: {
        ...
        price: 888,
        description:
          'IU HOLSSI IU HOLSSI IU HOLSSI IU HOLSSI IU HOLSSI IU HOLSSI',
        images: [
          'https://user-images.githubusercontent.com/35371660/230712070-afa23da8-1bda-4cc4-9a59-50a263ee629f.png',
          'https://user-images.githubusercontent.com/35371660/230711992-01a1a621-cb3d-44a7-b499-20e8d0e1a4bc.png',
          'https://user-images.githubusercontent.com/35371660/230712056-2c468ef4-45c9-4bad-b379-a9a19d9b79a9.png',
...
it('장바구니에 포함된 아이템들의 이름, 수량, 합계가 제대로 노출된다', async () => {
  ...
  expect(within(firstItem).getByText('IU SHOPPER')).toBeInTheDocument();
  expect(within(firstItem).getByRole('textbox')).toHaveValue('300');
  expect(within(firstItem).getByText('$125,100.00')).toBeInTheDocument();
...
it('특정 아이템의 수량이 변경되었을 때 값이 재계산되어 올바르게 업데이트 된다', async () => {
  ...
  const input = within(secondItem).getByRole('textbox');
...
it('특정 아이템의 삭제 버튼을 클릭할 경우 해당 아이템이 사라진다', async () => {
  ...
  expect(screen.getByText('IU SHOPPER')).toBeInTheDocument();

  await user.click(deleteButton);

  expect(screen.queryByText('IU SHOPPER')).not.toBeInTheDocument();
  // 삭제한거 확인할 땐 queryByText 로 확인하기
});
